...
if( $_GET['msg'] == "motoboy" ) {
    $type = $_GET['msg'];
    $id = $_GET['id'];
    $comprovante = mysqli_query( $db_con, "SELECT * FROM pedidos  WHERE id = '$id' ");
    $comprovante = mysqli_fetch_array($comprovante);
    $comprovante = urlencode($comprovante['comprovante']);
    echo '<script>';
	...
	echo "<script>var refreshTimeout;

	$(document).ready(function() {
		// Quando o modal é aberto
		$('#modalwhats').on('show.bs.modal', function (e) {
			// Pausa a atualização da página
			clearTimeout(refreshTimeout);
		})
	
		// Quando o modal é fechado
		$('#modalwhats').on('hide.bs.modal', function (e) {
			// Retoma a atualização da página
			refreshTimeout = setTimeout(function() {
				location.reload();
			}, 60000); // 60 segundos
		})
	
		// Inicia a atualização da página
		refreshTimeout = setTimeout(function() {
			location.reload();
		}, 60000); // 60 segundos
	});
